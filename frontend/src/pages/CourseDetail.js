...
const normalizeCommand = (cmd) => {
  ...
  if (c.includes("summarize") || c.includes("summarise") ||
      c.includes("summary"))                                  return "summarize";
  if (c.includes("explain") || c.includes("simple") ||
      c.includes("easy"))                                     return "simple";
  if (c.includes("example") || c.includes("real") ||
      c.includes("instance"))                                 return "example";
...
const CourseDetail = () => {
  ...
  const speakOrientation = (courseData) => {
    ...
    if (!c) return;
    const userName = localStorage.getItem("username") || "Student"; // ← fixed key
    speak(
      `Welcome ${userName}. ` +
      `You are inside the course ${c.title}. ` +
      `It has ${c.modules.length} modules and ${c.quizzes.length} quizzes. ` +
      `Currently on module ${currentRef.current + 1}. ` +
      `Say next to go to the next module. ` +
      `Say repeat to hear the current module again. ` +
      `Say quiz to start the quiz. ` +
      `Say assistant to open A I help. ` +
      `Say where am I to know your position. ` +
    ...
    const handleCommand = (transcript) => {
      ...
      if (modNow === "learn") {
        ...
        if (action === "next") {
          if (curNow + 1 < c.modules.length) {
            ...
            speak(
              ...
              `${c.modules[next]}. ` +
              `Say next, repeat, quiz, or assistant.`
            );
          } else {
            speak(
              `You have reached the end of all modules. ` +
              `Say quiz to start the quiz, or repeat to hear the last module again.`
            );
          }
          return;
        }
        ...
        speak(
          ...
          `Say next, repeat, quiz, assistant, or help.`
        );
      }
    };
